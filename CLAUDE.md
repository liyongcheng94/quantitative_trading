...
## 项目概述
两个组件：
1. **qtrader** — 零依赖 A股量化回测框架：双均线 / 海龟 / 网格 三策略横向对比，向量化引擎，无 K线 for 循环；含交易成本模型、parquet 数据缓存、参数网格搜索 + Walk-Forward 验证
2. **learning** — 系统化学习教材：Phase 1「打地基」10 节 + Phase 2「策略与组合」5 节互动 Jupyter 课程（金融通识 + Pandas/NumPy + 向量化 + 三策略 + 选股 + 组合）
...
## 核心命令
- **运行测试**：`.venv/Scripts/python.exe -m pytest tests/ -v`（52 tests）
...
- **参数网格搜索**：`.venv/Scripts/python.exe -m scripts.run_gridsearch --code 002594`
...
- **重生成 Phase 2 全部 notebook**：`.venv/Scripts/python.exe learning/phase2_strategies/_build_phase2.py`
- **安装依赖**：`.venv/Scripts/python.exe -m pip install -r requirements.txt`

## 代码地图
- `qtrader/` — 回测主包
  - `data.py` — akshare 数据层（新浪源主、东财源备）
  - `cache.py` — parquet 本地数据缓存（`~/.qtrader_cache/`），命中即用，未命中联网抓并回写
  - `strategies.py` — `Strategy` 基类 + `DualMAStrategy` / `TurtleStrategy`（含 ATR 仓位）/ `GridStrategy`
  - `engine.py` — `BacktestEngine` + `BacktestResult` + `CostModel`（成本模型，含印花税/佣金/过户费）
  - `metrics.py` — `compute_metrics`（15 指标：total/ann return、max_drawdown+date、sharpe、sortino、calmar、volatility、win_rate、profit_factor、exposure_time、information_ratio、bench_total、final_nav、n_days）
  - `optimize.py` — 参数优化（`grid_search` 网格搜索 + `walk_forward` 走查验证，防过拟合）
  - `plotting.py` — `print_comparison_table` + `plot_comparison`
- `scripts/` — CLI 入口（`run_backtest` / `run_dual_ma` / `run_gridsearch` / `build_report`）
- `tests/` — pytest 单元测试（52 tests，合成数据，不联网）
...
- `learning/` — 系统化学习教材
  ...
  - `phase1_foundation/` — 阶段一（1.5–2 周，10 节 notebook）
    ...
    - `exercises/solutions/exNN.py` — 9 份 UTF-8 可独立运行的参考答案
  ...
  - `phase2_strategies/` — 阶段二（1–1.5 周，5 节 notebook）
    - `11_dual_ma_timing.ipynb` ~ `15_portfolio_construction.ipynb` — 双均线/海龟/网格/多股选股/组合构建
    - `exercises/ex14.py` / `ex15.py` — 选股与组合练习
    - `_build_phase2.py` — notebook 生成器（改内容后重跑）
...
7. **改 notebook 内容**：编辑对应阶段的 `_build_*.py` 生成器脚本后重跑，**不要直接编辑 .ipynb**（会被下次重生成覆盖）
8. **notebook sys.path 自检**：notebook 首格已写自动定位 phase1_foundation 目录，兼容"从项目根启动 jupyter lab"和"从 phase1_foundation 启动"两种 CWD
9. **费率/规则类硬事实**：2026 已核实仍准确（印花税 0.05%、过户费 0.001%、涨跌停板）；2026 已更新（佣金万 2.1 免 5、无风险利率 rf=1.75%（10年期国债收益率，2026-06 ≈ 1.71%–1.76%）、量化高频监管）
...
- [learning/INDEX.md](learning/INDEX.md) — 15 节课目录与进度
- [learning/phase1_foundation/00_setup.ipynb](learning/phase1_foundation/00_setup.ipynb) — Phase 1 第一节课（环境自检）
- [learning/phase2_strategies/11_dual_ma_timing.ipynb](learning/phase2_strategies/11_dual_ma_timing.ipynb) — Phase 2 第一节课（双均线择时）
